...
function applySafeItems(gangs, safeItems, authoritative = false) {
  if (!authoritative && !safeItems?.length) return gangs;
  ...
  return gangs.map((gang) => ({
    ...gang,
    ...(authoritative || itemsByGang.has(gang.id)
      ? { safeItems: itemsByGang.get(gang.id) || [] }
      : {}),
  }));
}

function applySafeLogs(gangs, safeLogs, authoritative = false) {
  if (!authoritative && !safeLogs?.length) return gangs;
  ...
  return gangs.map((gang) => ({
    ...gang,
    ...(authoritative || logsByGang.has(gang.id)
      ? { safeLogs: logsByGang.get(gang.id) || [] }
      : {}),
  }));
}
...
export async function loadSharedStore() {
  ...
  const hasSafeItemsTable = !safeItemsError || safeItemsError.code !== "42P01";
  const hasSafeLogsTable = !safeLogsError || safeLogsError.code !== "42P01";
  const local = readLocalStore();
  const merged = {
    ...local,
    gangs: applySafeLogs(
      applySafeItems(data?.gangs || [], safeItems, hasSafeItemsTable),
      safeLogs,
      hasSafeLogsTable,
    ),
    checks: data?.checks || [],
